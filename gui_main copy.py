...
class App(tkinter.Tk):
    ...
    def initCenter(self):
        ...
        tkinter.Label(self.frameGroup, text='Or').grid(row=2, column=0)

        tkinter.Label(self.frameGroup, text='PhoneNumber').grid(row=3, column=0)
        self.phonenumber = tkinter.Text(self.frameGroup, width=30, height=10)
        self.phonenumber.grid(row=4, column=0, sticky=tkinter.EW)
        # self.phonenumber.bind('<Return>', self.send_message)
        self.phonenumber.insert(tkinter.END, "+85266125259")


        tkinter.Button(self.frameGroup, text='添加群组链接',
                command=self.addGroupLink).grid(row=5, column=0)
    ...
    def add_account(self, event=None):
        ...
        saveData(str(account.id), username)
    ...
    async def send_message_by_schedule(self):
        ...
        self.scheduler.add_listener(self.schedulerListener, mask=base.STATE_STOPPED) # 任务监听
    ...
    def addGroupLinkHandle(self):
        print("群组链接: ", self.myGroupLink.get().strip())
